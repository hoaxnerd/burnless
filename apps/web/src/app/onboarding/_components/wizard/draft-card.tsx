...
export function DraftCard({
  ...
  return (
    <div
      className={`relative mb-2.5 flex items-center gap-3 rounded-lg border border-surface-200 bg-surface-0 px-3.5 py-3 dark:border-surface-700 dark:bg-surface-800${
        ai ? " border-l-[3px] border-l-accent-500" : ""
      }`}
    >
      <div className="min-w-0 flex-1">
        <div className="truncate text-sm font-semibold text-surface-900 dark:text-surface-100">{title}</div>
        {meta ? <div className="mt-0.5 text-xs text-surface-500">{meta}</div> : null}
      </div>
      {amount ? (
        <span className="text-sm font-bold tabular-nums text-surface-900 dark:text-surface-100">{amount}</span>
      ) : null}
      {onEdit ? (
        <button
          type="button"
