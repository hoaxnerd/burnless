...
/**
 * SharedFormulaViewer — app-wide formula viewer that reads from MetricsContext.
 * Shows a metric's formula, dependency tree, and what depends on it.
 * Available on all pages (not just dashboard).
 */
...
export function SharedFormulaViewer() {
  const ctx = useOptionalMetrics();
  const formulaViewerSlug = ctx?.formulaViewerSlug ?? null;
  const closeFormulaViewer = ctx?.closeFormulaViewer ?? (() => {});
