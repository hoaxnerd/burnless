...
import type { Message, PendingPermission, PendingInput, PendingPlan } from "@/app/(dashboard)/ai/_components/types";
...
export function ChatSessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const submitPlan = useCallback(async (id: string, pending: PendingPlan, plan: PendingPlan["spec"]) => {
    const key = keyOf(id);
    patchLast(key, (m) => ({ ...m, pendingPlan: m.pendingPlan ? { ...m.pendingPlan, resolved: true } : null, isStreaming: true }));
    write(key, (s) => ({ ...s, isLoading: true }));
    try {
