...
- **🚫 NEVER hand-author / arbitrary migration SQL. ALWAYS generate migrations through drizzle-kit (`db:generate`).** Migrations are `db:push` for dev (no files), `db:generate` → `db:migrate` for prod. The SQL files in `packages/db/drizzle/` and `drizzle/meta/{_journal.json,NNNN_snapshot.json}` are drizzle-kit-owned artifacts — do not write or edit them by hand. Hand-authoring leaks (no snapshot → `generate` can't diff; filename-number collisions → out-of-order/duplicate migrations; journal drift → `db:migrate` silently skips them). If `db:generate` is "blocked by malformed meta," **FIX the meta/snapshots so drizzle-kit works again — do NOT work around it by hand-writing `ALTER` SQL.** History debt to repair before the next schema change: migrations `0043`–`0049` were hand-authored and their drizzle-kit snapshots (`0043`–`0049`) are MISSING. The two filename-number collisions are now fixed — the two most-recently-created files were renamed to sequential `0048_ai_write_scenario_id` / `0049_forecast_line_subcategory`, so filename order == journal order == true creation-time order. `_journal.json` is reconciled (idx 40–46, ordered by creation `when`, NOT filename number) so prod `db:migrate` applies `0043`–`0049`; but `db:generate` stays broken until the missing snapshots are rebuilt and the pre-existing 0017/0018 + 0041/0042 meta is repaired. Keep `db:push` (dev) and migration files in sync before shipping.
