...
- **`generateCashFlow.fundingInflows` is deprecated (Phase 3 F §F6).** Phase 2 D's `fundingImpact` (5th arg) subsumes it. Both call sites (`/api/statements` and `compute-dashboard.ts`) now pass `fundingImpact`; `fundingInflows` is retained as a JSDoc `@deprecated` parameter and silently ignored when both are supplied. New code MUST pass `fundingImpact`.
